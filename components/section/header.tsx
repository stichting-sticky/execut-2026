...
export function Header({ config, event }: HeaderProps) {
...
function DesktopHeader({ pages }: { pages: HeaderPage[] }) {
    return (
        <header
            id="site-header-desktop"
            className="sticky top-0 z-50 hidden w-screen items-center justify-between bg-background md:flex"
        >
            <div className="flex items-center gap-4 bg-secondary px-4 py-6 md:px-16 xl:py-8">
                <Logo />
            </div>
...
function MobileHeader({
    ...
    return (
        <header
            ...
            className="sticky top-0 z-50 flex w-full items-center justify-between bg-secondary px-6 py-4 md:hidden"
        >
            <Link href="/">
                <Logo small />
            </Link>
...
function MobileMenu({
    ...
    return (
        <div
            className={`fixed inset-0 z-40 flex flex-col bg-secondary pt-20 transition-transform duration-300 ease-in-out md:hidden ${
            ...
            <div className="flex flex-1 flex-col px-6 py-8">
                ...
                {showTickets && event?.tickets?.url && (
                    <div className="mt-auto pb-8">
                        <Button
                            variant="default"
                            className="w-full bg-background py-6 text-xl text-secondary hover:bg-background/90"
                            asChild
                        >
                            <Link
...
function useHeaderHeight() {
    useLayoutEffect(() => {
        const desktop = document.getElementById("site-header-desktop");
        const mobile = document.getElementById("site-header-mobile");
